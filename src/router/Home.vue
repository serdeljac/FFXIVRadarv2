<template>
  <div class="home">
      ...
      <PageHeader :title="`FFXIV Radar`" :tagline="pageTagLine" icon="eorzeamap" :ishome="true"/>
      ...
      <div class="main-grid">
        ...
        <div class="quick-links">
          ...
          <nav class="nav-grid">
            <router-link
              v-for="(l, index) in linkList" :key="l.href"
              :to="l.href"
              :style="{ '--i': index }"
              :class="[`nav-btn`, {'featured': l.feature}]">
                <navIcon :name="l.iconName"/>
                <span class="nav-label">{{ l.displayName }}</span>
              </router-link>
          </nav>
        </div>
      </div>
      ...
      <footer class="page-footer">
        ...
        <p class="footer-copy">© 2025–{{ currentYear }} FFXIV Radar.</p>
...
<script lang="ts" setup>
  ...
  import { linkList } from '../layouts/sidebar/SidebarLinks.ts'
  const currentYear = new Date().getFullYear()
  const pageTagLine = 'Your aetheric compass for Eorzea'
</script>
